...
#endif
#include "rtklib.h"
//#include <windows.h> // Include Windows-specific header for SYSTEMTIME and GetSystemTime
#include <time.h>
#include <sys/time.h>
...
#endif
// 深拷贝 gtime_t 结构体
gtime_t deepCopyGtime(const gtime_t *src) {
    gtime_t dest = *src; // 假设 gtime_t 可以直接复制
    return dest;
}

// 深拷贝 obsd_t 结构体
obsd_t deepCopyObsd(const obsd_t *src) {
    obsd_t dest = *src; // 直接复制所有成员
    dest.time = deepCopyGtime(&src->time); // 深拷贝 time 成员
    return dest;
}

// 深拷贝 obs_t 结构体
obs_t deepCopyObs(const obs_t *src) {
    obs_t dest = *src; // 复制简单类型

    // 深拷贝 data 指针
    if (src->data != NULL) {
        dest.data = (obsd_t *)malloc(src->nmax * sizeof(obsd_t));
        if (dest.data == NULL) {
            fprintf(stderr, "内存分配失败\n");
            exit(1); // 或者处理错误
        }
        for (int i = 0; i < src->n; i++) {
            dest.data[i] = deepCopyObsd(&src->data[i]);
        }
    } else {
        dest.data = NULL;
    }
...
// 深拷贝 nav_t 结构体
nav_t deepCopyNav(const nav_t *src) {
    nav_t dest = *src; // 假设 nav_t 可以直接复制
    return dest;
}

// 深拷贝 sta_t 结构体
sta_t deepCopySta(const sta_t *src) {
    sta_t dest = *src; // 假设 sta_t 可以直接复制
    return dest;
}

// 深拷贝 dgps_t 结构体
dgps_t* deepCopyDgps(const dgps_t *src) {
    if (src == NULL) return NULL;
    dgps_t *dest = (dgps_t *)malloc(sizeof(dgps_t));
    if (dest == NULL) {
        fprintf(stderr, "内存分配失败\n");
        return NULL;
    }
    *dest = *src; // 假设 dgps_t 可以直接复制
    return dest;
}

// 深拷贝 rtcm_t 结构体
rtcm_t* deepCopyRTCM(const rtcm_t *src) {
    if (src == NULL) return NULL;

    // 分配内存
    rtcm_t *dest = (rtcm_t *)malloc(sizeof(rtcm_t));
    if (dest == NULL) {
        fprintf(stderr, "内存分配失败\n");
        return NULL;
    }
    ...
    *dest = *src;
    ...
    // 深拷贝嵌套结构体
    dest->time = deepCopyGtime(&src->time);
    dest->time_s = deepCopyGtime(&src->time_s);
    dest->obs = deepCopyObs(&src->obs);
    dest->nav = deepCopyNav(&src->nav);
    dest->sta = deepCopySta(&src->sta);

    // 深拷贝指针成员
    dest->dgps = deepCopyDgps(src->dgps);

    // 深拷贝 ssr 数组
    for (int i = 0; i < MAXSAT; i++) {
        dest->ssr[i] = src->ssr[i]; // 假设 ssr_t 可以直接复制
    }
...
extern int sortobs(obs_t *obs)
{
    ...
    obsd_t tempdata[2000];
    for (int i = 0; i < 2000; i++) {
        tempdata[i] = obs->data[i];
    }
